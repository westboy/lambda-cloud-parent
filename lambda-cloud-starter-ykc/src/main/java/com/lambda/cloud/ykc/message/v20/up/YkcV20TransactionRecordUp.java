...
public class YkcV20TransactionRecordUp {
    ...
    @ProtocolField(order = 21, length = 16, composite = true, dataType = ProtocolDataType.LIST,
            listElementType = ProtocolDataType.COMPOSITE, listElementClass = YkcV20TransactionRatePeriod.class,
            listElementSizeField = "ratePeriodCount", description = "费率时段列表")
    private List<YkcV20TransactionRatePeriod> ratePeriods;

    @ProtocolField(order = 22, length = 4, composite = true, dataType = ProtocolDataType.LIST,
            listElementType = ProtocolDataType.COMPOSITE, listElementClass = YkcV20HalfHourEnergy.class,
            listElementSize = 48, description = "半小时电量列表")
