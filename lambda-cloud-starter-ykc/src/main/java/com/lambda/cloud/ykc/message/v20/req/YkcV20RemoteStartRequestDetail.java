...
@Setter
@ToString
@ProtocolFrame(frameType = "A8", name = "运营平台远程控制启机", description = "运营平台远程控制启机请求详细信息")
public class YkcV20RemoteStartRequestDetail {
    ...
    private byte[] physicalCardNumber;

    /**
     * 账户余额 (4字节)
     * 保留到小数点两位
     */
    @ProtocolField(order = 6, length = 4, dataType = ProtocolDataType.UINT32, description = "账户余额")
     ...
     */
    @ProtocolField(order = 7, length = 2, dataType = ProtocolDataType.UINT16, description = "本次充电当前允许的最大功率")
     ...
     * 精确到小数点后四位；默认全0，不限制
     */
    @ProtocolField(order = 9, length = 4, dataType = ProtocolDataType.UINT32, description = "充电电量限制")
