### lamuda-cloud-starter-websocket 项目介绍及使用说明

#### 简介
`lamuda-cloud-starter-websocket` 是一个基于WebSocket的实时通信模块，主要用于在微服务架构中实现实时通信功能。它包含了WebSocket的自动配置、消息发送、消息接收等功能，以简化开发者在项目中对WebSocket的管理。
...
#### 使用方式
要在你的项目中使用 `lamuda-cloud-starter-websocket`，你需要在项目的pom文件中添加以下依赖：
```xml
<dependency>
    <groupId>${project.groupId}</groupId>
    <artifactId>lamuda-cloud-starter-websocket</artifactId>
...
通过上述配置和依赖添加，你可以在项目中使用`lamuda-cloud-starter-websocket`提供的实时通信功能，简化WebSocket的配置和使用。