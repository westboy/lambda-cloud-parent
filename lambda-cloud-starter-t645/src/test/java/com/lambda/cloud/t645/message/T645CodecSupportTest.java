...
class T645CodecSupportTest {
    ...
    void testSubOffset33() {
        ...
        assertEquals((byte) 0xDE, result2[0]);
    }
    ...
    void testReverseAddress() {
        ...
        Exception exception =
                assertThrows(IllegalArgumentException.class, () -> T645CodecSupport.reverseAddress("123"));
        assertTrue(exception.getMessage().contains("地址域长度必须为12个十六进制字符"));

        assertThrows(IllegalArgumentException.class, () -> T645CodecSupport.reverseAddress(null));
    }
    ...
    void testBuildDiKey() {
        ...
        assertEquals("91:C0320000", T645CodecSupport.buildDiKey(0x91, "c0320000"));
        assertEquals("00:NONE", T645CodecSupport.buildDiKey(0x00, null));
        assertEquals("80:NONE", T645CodecSupport.buildDiKey(0x80, ""));
    }
}
