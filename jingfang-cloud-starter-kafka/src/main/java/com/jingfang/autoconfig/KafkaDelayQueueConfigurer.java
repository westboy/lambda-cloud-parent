...
import com.jingfang.cloud.kafka.delayqueue.DelayConsumerRecord;
import com.jingfang.cloud.kafka.delayqueue.DelayKafkaInitializer;
import com.jingfang.cloud.kafka.delayqueue.DelayMonitorService;
import com.jingfang.cloud.kafka.delayqueue.DelayTimeoutService;
...
public class KafkaDelayQueueConfigurer {
    ...
    public Executor kafkaNoboundTaskExecutor() {
        ...
        //线程名字前缀
        executor.setThreadNamePrefix("jinfang-cloud-delay-kafka-bound-");
    ...
    public Executor kafkaMmaxfixedTaskExecutor() {
        ...
        executor.setKeepAliveSeconds(300);
        //线程名字前缀
        executor.setThreadNamePrefix("jinfang-cloud-delay-kafka-fixed-");
        executor.setRejectedExecutionHandler(new ThreadPoolExecutor.CallerRunsPolicy());
        executor.setWaitForTasksToCompleteOnShutdown(true);
        executor.initialize();
        return executor;
    ...
    @Bean
    public DelayKafkaInitializer delayKafkaCommandRunner(DelayMonitorService delayMonitorService,
    ...
    public NewTopic defaultDelayedTopics() {
        return TopicBuilder.name(DelayConsumerRecord.DELAY_TOPIC).partitions(SIZE).replicas(1).build();
    }
}
